...
class PhotoImportController extends Controller
{
    ...
    public function import()
    {
        ...
        DB::table('photo_photo_category')->truncate();
        Photo::truncate();
        ...
        foreach ($rows as $index => $row) {
            ...
            $data[] = [
                'title'      => $row->name,
                'slug'       => $row->alias,
                'template'   => FullTemplate::Photo->value,
                'img'        => $row->introimage ?? null,
        ...
        return response()->json([
            'status'   => 'ok',
            'imported' => count($data),
            'sample'   => DB::table('photos')->orderByDesc('created_at')->limit(5)->get(['id', 'title', 'slug', 'created_at']),
        ], 200, [], JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
    }
}
